...
{
    // 定义基础指令
    const QUEUE_LISTEN = 'xadmin:service listen';
    ...
    {
        $this->setName('xadmin:service');

        $this->addArgument('name', Argument::OPTIONAL, 'stop|start|status|query|clean|listen|webstop|webstart|webstatus', 'listen');
        ...
        $this->setDescription('Background Daemons Management, Argument: stop|start|query|listen|webstop|webstart|webstatus');
    }
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $this->output->error("># Wrong operation, Allow stop|start|status|query|clean|listen|webstop|webstart|webstatus");
    }
    ...
    {
        if (count($result = $this->process->thinkQuery('xadmin:service')) < 1) {
    ...
    {
        $list = $this->process->thinkQuery('xadmin:service');
    ...
    protected function listenAction()
    {
        ...
        while (true) {
            ...
            foreach (SystemQueue::mk()->where($map)->order('exec_time asc')->cursor() as $vo) try {
                $args = "xadmin:service dorun {$vo['code']} -";
    ...
    protected function doRunAction()
    {
        ...
        if (empty($this->code)) {
            ...
            if (empty($this->queue->record) || intval($this->queue->record['status']) !== 1) {
                ...
                $this->queue->progress(2, '>>> 任务处理开始 <<<', '0.00');
                // 执行任务内容
